...
//
// REWRITTEN — Full feature set:
// ✅ User switcher dropdown (all registered users)
// ✅ Selected user's activity (AI Scans, Near Miss, Entries, Resolved)
// ✅ Severity breakdown (Critical / High / Medium / Low) with bars
// ✅ Plant-wise list — all SAIL plants, each row tappable
// ✅ Every number hyperlinked → opens filtered case sheet
// ✅ Data from LocalDB (instant) + Sheets refresh in background
...
class _DashboardTabState extends State<DashboardTab> {
  ...
  // ── Load local data first, then refresh from Sheets ───────────
  Future<void> _loadAll() async {
    await _loadLocal();
    _refreshFromSheets(); // background, no await
  }
  ...
  List<Map<String, dynamic>> _fallbackUsers() {
    // Shows logged-in user + placeholder so dropdown is never empty
    final u = widget.user;
    if (u == null) return [];
    return [u];
  }
  ...
  int get _resolved  => _userIncidents.where((i) =>
      ...
      (i['status']?.toString().toUpperCase() ?? '') == 'RESOLVED').length;
  ...
  int get _medium    => _userIncidents.where((i) =>
      (i['severity']?.toString().toUpperCase() ?? '') == 'MEDIUM' ||
      (i['severity']?.toString().toUpperCase() ?? '') == 'MODERATE').length;
  ...
  Map<String, Map<String, int>> _plantStats() {
    ...
    for (final p in _plants) {
      result[p['code']!] = {
        'total': 0, 'open': 0, 'critical': 0, 'high': 0, 'scans': 0
      };
    }
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              items: _allUsers.map((u) {
                ...
                final code  = plant.contains(' ')
                    ? plant.split(' ').first : plant;
                return DropdownMenuItem<String>(
                  value: key,
                  child: Row(children: [
                    Expanded(child: Text(uname,
  ...
  Widget _buildSeveritySection(SL sl) {
    final max = [_critical, _high, _medium, _low]
        .fold(1, (a, b) => a > b ? a : b);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _severityRow(
    String label, int count, Color color, int max, SL sl,
  ...
  Widget _buildQuickActions(SL sl) => Column(
    ...
    children: [
      ...
      const SizedBox(height: 10),
      Row(children: [
        _actionBtn(Icons.document_scanner_rounded,
        ...
        const SizedBox(width: 8),
        _actionBtn(Icons.bar_chart_rounded,
  ...
  Widget _buildPlantSection(SL sl) {
    ...
    // Sort: my plant first, then by total descending
  ...
  Widget _plantRow(String code, String name, Map<String, int> s,
      bool isMy, SL sl) {
  ...
  void _showPlantSheet(String code, String name,
      Map<String, int> stats, SL sl) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _PlantSheet(
        code: code, name: name, stats: stats,
        cases: plantCases, sl: sl,
      ),
    );
  }

  /// All plants grid sheet
  void _showAllPlantsSheet(
      Map<String, Map<String, int>> stats, String myCode, SL sl) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
...
class _CaseCard extends StatelessWidget {
  ...
  final SL sl;
  const _CaseCard({required this.inc, required this.sl});
  ...
  Widget build(BuildContext context) {
    ...
    final title    = inc['title']?.toString() ?? inc['desc']?.toString() ?? 'Untitled';
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 6, runSpacing: 4, children: [
          _pill(sev, sevColor),
          _pill(status,
            status.toUpperCase() == 'OPEN' ? AppColors.amber : AppColors.green),
