...
Respond ONLY with the JSON — no explanations outside JSON.''';
  ...
  Future<void> _shareViaWhatsApp(Map<String, dynamic> incident) async {
    ...
    // First check if pdfUrl already exists (from background upload)
    ...
    if (pdfUrl == null || pdfUrl.isEmpty) {
      ...
      try {
        ...
        final pdfBytes = await PdfExport.generateIncidentReportBytes(
          ...
          reporterName: user?['name']?.toString() ?? 'SAIL Safety Officer',
          reporterPno:  user?['pno']?.toString()  ?? '',
          imageBytes:   null, // image already cleared after save
        );
    ...
    if (pdfUrl != null && pdfUrl.isNotEmpty) {
      text = '⚠️ *SAIL Safety Lens — Near Miss Report*\n\n'
          '📋 *Title:* $title\n'
          '🔴 *Severity:* $severity\n'
          '🏭 *Plant:* $plant\n'
          '📅 *Date:* $date\n\n'
      ...
      // Fallback to text-only if PDF generation failed
