...
👷 Reported via Safety Lens App''';
  ...
  Future<bool> _submit({bool exportAfter = false}) async {
    ...
    try {
      ...
      // ★ v30 FIX: Preserve image bytes BEFORE clearing, for share dialog
  ...
  Future<void> _shareViaWhatsApp(Map<String, dynamic> incident, [Uint8List? savedImageBytes]) async {
    // ★ v30: Share PDF + image via WhatsApp using Share.shareXFiles
    ...
    try {
      ...
      final pdfBytes = await PdfExport.generateIncidentReportBytes(
        ...
        reporterName: user?['name']?.toString() ?? 'SAIL Safety Officer',
        reporterPno:  user?['pno']?.toString()  ?? '',
        imageBytes:   savedImageBytes, // ★ v30 FIX: use preserved image bytes
      );
      ...
      final text = '⚠️ *SAIL Safety Lens — Near Miss Report*\n\n'
          '📋 *Title:* $title\n'
          '🔴 *Severity:* $severity\n'
          '🏭 *Plant:* $plant\n'
          '📍 *Location:* $location\n'
      ...
      if (!kIsWeb && pdfBytes.isNotEmpty) {
        // Save PDF + image to temp files and share via shareXFiles
        final tempDir = await getTemporaryDirectory();
        final files = <XFile>[];
        ...
        // Also add image if available (so recipient sees both)
        ...
        await Share.shareXFiles(files, text: text, subject: 'Near Miss Report — $plant');
      } else {
        // Fallback: try uploading PDF to Drive and share link
        String? pdfUrl = incident['pdfUrl']?.toString();
        if ((pdfUrl == null || pdfUrl.isEmpty) && pdfBytes.isNotEmpty) {
        ...
        final shareText = pdfUrl != null && pdfUrl.isNotEmpty
            ? '$text\n\n📄 *Full PDF Report:*\n$pdfUrl'
            : text;
