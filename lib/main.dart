...
// ─── DESIGN TOKENS ────────────────────────────────────────────────────────────
// Static colors that never change (severity, brand accents)
class AppColors {
  // Static fields used by existing screens (dark theme values)
  ...
  // Brand
  static const accent     = Color(0xFF6C63FF); // electric violet
...
// ─── THEME HELPER ─────────────────────────────────────────────────────────────
// Use SL.of(context).bg etc instead of hardcoded colors
class SL {
  final bool isDark;
...
class _SafetyLensAppState extends State<SafetyLensApp> {
  ...
  ThemeData _buildTheme(bool dark) {
    ...
    return base.copyWith(
      ...
      colorScheme: (dark
        ? const ColorScheme.dark(primary: AppColors.accent, surface: AppColors.darkCard)
        : const ColorScheme.light(primary: AppColors.accent, surface: Colors.white)
      ...
      bottomNavigationBarTheme: BottomNavigationBarThemeData(
        ...
        unselectedItemColor: dark
          ? const Color(0xFF64748B) : const Color(0xFF9898C0),
      ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: border ?? Border.all(
          color: sl.border.withOpacity(0.6), width: 1),
        ...
        decoration: BoxDecoration(
          ...
          color: onTap == null
            ? (SL.of(context).isDark
                ? AppColors.darkCard2 : AppColors.lightBorder)
            : null,
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(colors: [
        Colors.transparent, color.withOpacity(0.6), Colors.transparent])));
}
