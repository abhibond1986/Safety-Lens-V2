...
class PdfExport {
  ...
  /// Share the PDF via WhatsApp / Email / etc.
  ...
  static pw.Widget _buildPageFooter(int pageNo, int total) {
    return pw.Container(
      ...
      alignment: pw.Alignment.center,
      child: pw.Text(
        'Page $pageNo of $total · Generated by Safety Lens · SAIL',
        style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey),
      ),
    );
  }

  static pw.Widget _buildIncidentInfo(
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          _kvRow('Plant', incident['plant'] ?? 'Not specified'),
          _kvRow('Department', incident['dept'] ?? 'Not specified'),
          _kvRow('Location', incident['location'] ?? 'Not specified'),
          _kvRow('Date / Time', dateStr),
          _kvRow('Reported by', '$reporterName (P.No: $reporterPno)'),
          _kvRow('Type', incident['type'] == 'AI_SCAN' ? 'AI Image Scan' : 'Near Miss Report'),
          _kvRow('Status', incident['status'] ?? 'OPEN'),
        ],
      ),
    );
  }
  ...
  static pw.Widget _buildSeverityCard(String severity) {
    ...
    return pw.Container(
      padding: const pw.EdgeInsets.all(12),
      decoration: pw.BoxDecoration(
        color: PdfColor.fromHex(color).shade(0.1),
        border: pw.Border.all(color: PdfColor.fromHex(color), width: 1.5),
        borderRadius: const pw.BorderRadius.all(pw.Radius.circular(6)),
      ),
      child: pw.Row(
        children: [
  ...
  static pw.Widget _buildWsaSection(Map<String, dynamic> incident) {
    final wsa = incident['wsa']?.toString() ?? '';
    if (wsa.isEmpty) return pw.SizedBox();
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
        ...
        pw.Container(
          padding: const pw.EdgeInsets.all(10),
          decoration: pw.BoxDecoration(
            color: PdfColor.fromInt(0xFFEEEDFE),
            borderRadius: const pw.BorderRadius.all(pw.Radius.circular(4)),
          ),
          child: pw.Text(
            wsa,
    ...
    return pw.Column(
      ...
      children: [
        ...
        pw.Container(
          ...
          child: pw.Text(
            incident['correctiveAction']?.toString() ??
                'Investigation pending. Corrective actions to be defined per IS 14489 audit framework.',
            style: const pw.TextStyle(fontSize: 11, lineSpacing: 1.4),
          ),
        ),
      ],
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.Text(
            'This report is generated as per IS 14489:1998 (Code of Practice on Occupational Safety & Health Audit for Iron & Steel Industry) and Ministry of Steel Safety Guidelines (2023). Factories Act 1948 §21-41 compliance verified.',
            style: const pw.TextStyle(fontSize: 8, lineSpacing: 1.3),
          ),
          pw.SizedBox(height: 6),
          pw.Text(
            'Generated by Safety Lens · SAIL Safety Organisation, Ranchi',
            style: pw.TextStyle(
              fontSize: 7,
  ...
  static pw.Widget _buildSeverityBreakdown(List<Map<String, dynamic>> incidents) {
    final byS = <String, int>{};
    for (final i in incidents) {
      final s = i['severity']?.toString() ?? 'MEDIUM';
      byS[s] = (byS[s] ?? 0) + 1;
    }
    return pw.Container(
      padding: const pw.EdgeInsets.all(10),
            ...
            pw.Row(
              children: [
                pw.Container(
                  width: 8,
                  height: 8,
                  color: PdfColor.fromHex(_severityColor(s)),
                ),
                pw.SizedBox(width: 6),
                pw.Text('$s: ${byS[s] ?? 0}', style: const pw.TextStyle(fontSize: 10)),
              ],
            ),
        ],
      ),
  ...
  static pw.Widget _buildIncidentTable(List<Map<String, dynamic>> incidents) {
    return pw.Table(
      ...
      children: [
        ...
        for (final i in incidents)
          pw.TableRow(children: [
            _td(DateFormat('dd/MM/yy').format(DateTime.parse(i['date']))),
            _td('${i['title'] ?? 'Unknown'}\n${(i['desc'] ?? '').toString().substring(0, (i['desc']?.toString().length ?? 0) > 60 ? 60 : (i['desc']?.toString().length ?? 0))}...'),
            _td('${i['plant'] ?? ''} · ${i['location'] ?? ''}'),
            _tdSeverity(i['severity']?.toString() ?? 'MEDIUM'),
            _td(i['reportedBy']?.toString() ?? 'Unknown'),
          ]),
      ],
    );
  }
  ...
  static pw.Widget _tdSeverity(String sev) {
    return pw.Padding(
      padding: const pw.EdgeInsets.all(6),
      child: pw.Container(
        padding: const pw.EdgeInsets.symmetric(horizontal: 4, vertical: 2),
        color: PdfColor.fromHex(_severityColor(sev)).shade(0.15),
        child: pw.Text(
          sev,
          style: pw.TextStyle(
            fontSize: 8,
            fontWeight: pw.FontWeight.bold,
            color: PdfColor.fromHex(_severityColor(sev)),
          ),
        ),
      ),
    );
  ...
  static String _severityColor(String sev) {
